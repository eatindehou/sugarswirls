...
for ($cpt = 0; $cpt < $pdosResultat->rowCount(); $cpt++) {
	...
	$strDate = $arrListe[$cpt]['echeance'];
	$timestamp = strtotime($strDate);
	$formatDate = date(' M d  ', $timestamp);
	// echo $formatDate;
	// $strFormat = "M";
		...
		<nav class="menuListe">
			<ul class="menuListe__listes">
				<?php for ($cptListe = 0; $cptListe < count($arrNomToutesLesListes); $cptListe++) { ?>
					<li style="border-left: .4rem solid <?php echo "#" . $arrNomToutesLesListes[$cptListe]["couleurs"]; ?>"
						class="menuListe__items"><a href="#"
							class="menuListe__lien"><?php echo $arrNomToutesLesListes[$cptListe]['nom'] ?></a></li>
				<?php } ?>
			</ul>
		</nav>
	</header>
